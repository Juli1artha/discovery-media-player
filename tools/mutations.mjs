...
export const MUTANTS = [
  ...
  {
    id: "compteurs-lus-par-une-requete-qui-se-compte",
    fichier: "charge/rapport.js",
    avant: "    const lireCompteurs = () => compteursDeLaCarte(player.__compteursSansObserver());",
    apres: "    const lireCompteurs = async () => compteursDeLaCarte(JSON.parse((await appeler({ method: \"GET\", headers: {}, socket: {}, query: { contract: \"1\" } }, { corpsEntier: true })).corps));",
    pourquoi: "c'est le défaut mesuré sur les trois artefacts du tag 0.1.168 : la requête qui LIT les compteurs traverse le handler et incrémente celui qu'elle mesure — delta ok valait 1 001 pour 1 000 requêtes, et `delta = after − before` restait vrai, donc rien ne le voyait",
    bancs: ["charge/__tests__/rapport.test.js"],
  },
  {
    id: "pic-de-cache-historique-pris-pour-celui-de-la-fenetre",
    fichier: "charge/rapport.js",
    avant: "        peakInFlight: vueCache.pic(), processLifetimePeakInFlight: cacheApres.peakInFlight,",
    apres: "        peakInFlight: cacheApres.peakInFlight, processLifetimePeakInFlight: cacheApres.peakInFlight,",
    pourquoi: "hits, misses et coalesced sont des deltas, mais le pic était recopié du maximum DEPUIS LE DÉMARRAGE : après une position chargée, une position calme héritait de l'ancien pic et affirmait une concurrence qu'elle n'avait pas vue",
    bancs: ["charge/__tests__/rapport.test.js"],
  },
  {
    id: "fenetre-de-mesure-ouverte-avant-le-prechauffage",
    fichier: "charge/rapport.js",
    avant: "    fenetre.processUptimeStartMs = horloge.uptimeMs();",
    apres: "    void 0;",
    pourquoi: "les trois bornes désignaient trois périodes différentes sous un seul nom : sur la course du tag, 4 057 ms d'uptime pour une durée annoncée de 4 002,231 ms, aux trois positions — un champ nommé « fenêtre de mesure » doit désigner UNE fenêtre",
    bancs: ["charge/__tests__/rapport.test.js"],
  },
  {
    id: "gigue-non-reproductible",
    fichier: "charge/rapport.js",
    avant: "  const alea = typeof graine === \"function\" ? graine : alea32(graine);",
    apres: "  const alea = typeof graine === \"function\" ? graine : Math.random;",
    pourquoi: "une graine enregistrée dans l'artefact mais non consommée est pire qu'aucune : elle affirme une reproductibilité que la course n'a pas, et deux campagnes du même protocole ne sont alors pas la même expérience",
    bancs: ["charge/__tests__/rapport.test.js"],
  },
  {
    id: "secret-publie-dans-un-artefact-d-echec",
    fichier: "charge/rapport.js",
    avant: "  t = t.replace(/\\b(authorization|apikey|api[-_]?key|token|secret|password|bearer|jwt)\\b[^,;\\n]*/gi, \"$1 «retiré»\");",
    apres: "  t = t.replace(/\\b(authorization|apikey|api[-_]?key|token|secret|password|bearer|jwt)\\b\\s*[:=]?\\s*\\S+/gi, \"$1 «retiré»\");",
    pourquoi: "cette forme ne mange qu'UN mot : sur « Authorization: Bearer sk-live-4242 » elle retire « Bearer » et publie le jeton juste derrière, sur une Release publique — une caviarderie qui laisse passer ce qu'elle vise donne l'assurance sans la protection",
    bancs: ["charge/__tests__/rapport.test.js"],
  },
  {
    id: "resume-produit-sur-une-cohorte-refusee",
    fichier: "tools/resume-de-charge.mjs",
    avant: "  if (verdict.code !== 0) {",
    apres: "  if (false) {",
    pourquoi: "un tableau bien présenté fait passer ses chiffres pour vérifiés : le produire sur une cohorte que le validateur refuse met la forme au service du faux",
    bancs: ["tools/__tests__/resumeDeCharge.test.js"],
  },
  {
    id: "resume-sans-la-provenance",
    fichier: "tools/resume-de-charge.mjs",
    avant: "    `**Course** \\`${id.runId}\\` · **version** \\`${id.packageVersion}\\` · **commit** \\`${id.commitSha}\\``,",
    apres: "    \"**Campagne de charge**\",",

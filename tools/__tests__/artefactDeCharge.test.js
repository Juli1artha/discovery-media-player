 ...
 */
const CLES_V1 = ["$defs.memoire.arrayBuffers","$defs.memoire.external","$defs.memoire.heapUsed","$defs.memoire.rss","$defs.percentiles.p50","$defs.percentiles.p95","$defs.percentiles.p99","$defs.percentilesEtMax.max","$defs.percentilesEtMax.p50","$defs.percentilesEtMax.p95","$defs.percentilesEtMax.p99","cache","cache.coalesced","cache.hits","cache.misses","cache.peakInFlight","cache.processLifetimePeakInFlight","complete","correctness","correctness.correctResponses","correctness.emptyResponses","correctness.wrongPresentation","counters","counters.after","counters.before","counters.delta","counters.observerOverheadRequests","database","database.calls","database.latencyMs","database.peakInFlight","database.timeouts","environment","environment.arch","environment.cpuCount","environment.cpuModel","environment.eventLoopResolutionMs","environment.memoryLimitMiB","environment.memoryLimitSource","environment.memorySampleIntervalMs","environment.node","environment.os","environment.postgresVersion","environment.postgrestVersion","environment.runnerImage","environment.runnerProvider","failure","failure.code","failure.phase","failure.reason","histogram","histogram.binSetId","histogram.counts","histogram.edges","histogram.overflow","histogram.unit","http","http.responseBytes","http.responseBytes.max","http.responseBytes.p50","http.responseBytes.p95","http.responseBytes.total","identity","identity.commitSha","identity.event","identity.packageVersion","identity.prHeadSha","identity.ref","identity.repository","identity.runAttempt","identity.runId","identity.runNumber","identity.schemaSha256","identity.timestamp","isolation","isolation.cacheReset","isolation.countersReportedAsDeltas","isolation.databaseReset","isolation.datasetId","isolation.metricsReset","isolation.sameProcessAcrossCohort","latencyMs","latencyMs.max","latencyMs.mean","latencyMs.min","latencyMs.n","latencyMs.p50","latencyMs.p95","latencyMs.p99","measurementWindow","measurementWindow.durationMs","measurementWindow.processUptimeEndMs","measurementWindow.processUptimeStartMs","measurementWindow.startedAt","process","process.cpuSystemMs","process.cpuUserMs","process.eventLoopP99Ms","process.memoryMiB","process.memoryMiB.afterGc","process.memoryMiB.baseline","process.memoryMiB.end","process.memoryMiB.peak","relay","relay.admitted","relay.bytesTransferred","relay.chunkBytes","relay.consumerDelayMs","relay.descriptors","relay.descriptors.idle","relay.descriptors.peak","relay.fileBytes","relay.highWaterMark","relay.refused","scenario","scenario.egressIps","scenario.name","scenario.position","scenario.presentations","scenario.repetition","scenario.sequence","scenario.spectators","scenario.warmupRequests","schemaVersion","statuses","statuses.2xx","statuses.429","statuses.503","statuses.other","statuses.other4xx","statuses.other5xx","topology","topology.clientToPlayer","topology.playerProcesses","topology.playerToDatabase","workload","workload.arrivalModel","workload.arrivalPattern","workload.completedRequests","workload.egressPattern","workload.generatorLagMs","workload.peakInFlight","workload.requestsPerSpectator","workload.scheduleAlgorithm","workload.scheduleSeed","workload.scheduledRequests","workload.startedRequests","workload.targetDurationMs"];
const REQUIS_V1 = ["schemaVersion", "complete", "failure", "identity", "environment", "scenario", "workload", "isolation", "measurementWindow", "topology"];
